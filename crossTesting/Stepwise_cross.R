...
# build best models using above coefs
...
lmodel_two <- lm(dataTwo_train$RetentionTime ~ unmodA + unmodC + unmodD + unmodE
              + unmodF + unmodG + unmodH + unmodI + unmodK + unmodL +
                + unmodM + unmodN + unmodP +unmodQ + unmodR + unmodS + unmodT + unmodV + unmodW + unmodY +
                + modS + modY + modT + modM , data = dataTwo_train)

# cross test
calcStats(dataTwo_test$RetentionTime, predict(lmodel_one, dataTwo_test))
calcStats(dataOne_test$RetentionTime, predict(lmodel_two, dataOne_test))