...
fit_lasso_cv_mse_one <- cv.glmnet(Xtrain_one, dataOne_train$RetentionTime, alpha = 1, nfolds = 5, foldid = foldid_one)
fit_lasso_cv_mse_two <- cv.glmnet(Xtrain_two, dataTwo_train$RetentionTime, alpha = 1, nfolds = 5, foldid = foldid_two)

bestModelminLambda_one <- glmnet(x = Xtrain_one,
                                 y = dataOne_train$RetentionTime,
                                 lambda = fit_lasso_cv_mse_one$lambda.min,
                                 alpha = 1)

bestModelOneSELambda_one <- glmnet(x = Xtrain_one,
                                   y = dataOne_train$RetentionTime,
                                   lambda = fit_lasso_cv_mse_one$lambda.1se,
                                   alpha = 1)

bestModelminLambda_two <- glmnet(x = Xtrain_two,
                                 y = dataTwo_train$RetentionTime,
                                 lambda = fit_lasso_cv_mse_two$lambda.min,
                                 alpha = 1)

bestModelOneSELambda_two <- glmnet(x = Xtrain_two,
                                   y = dataTwo_train$RetentionTime,
                                   lambda = fit_lasso_cv_mse_two$lambda.1se,
                                   alpha = 1)

# make testing matrices
...
predictions <- predict(bestModelminLambda_one, newx = Xtest_two)
calcStats(dataTwo_test$RetentionTime, predictions)

predictions <- predict(bestModelOneSELambda_one, newx = Xtest_two)
calcStats(dataTwo_test$RetentionTime, predictions)

# predict data one using model two
predictions <- predict(bestModelminLambda_two, newx = Xtest_one)
calcStats(dataOne_test$RetentionTime, predictions)

predictions <- predict(bestModelOneSELambda_two, newx = Xtest_one)
calcStats(dataOne_test$RetentionTime, predictions)
